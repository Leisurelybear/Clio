...
from clio.ratelimit import RateLimiter, make_rate_limiter
...
class TestRateLimiterContext:
    def test_first_call_no_wait(self):
        rl = RateLimiter(60)
        with patch("clio.ratelimit.time.monotonic", return_value=100.0):
            with patch("clio.ratelimit.time.sleep") as mock_sleep:
                with rl:
                    pass
    ...
    def test_logs_rate_limit_message(self, capsys):
        ...
        rl._next_at = 200.0
        with patch("clio.ratelimit.time.monotonic", return_value=100.0):
            with patch("clio.ratelimit.time.sleep"):
                with rl:
                    pass
