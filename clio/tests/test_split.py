"""Tests for clio/split.py — split_video."""
...
class TestSplitVideo:
    def test_no_split_when_under_max(self, monkeypatch, tmp_path: Path):
        monkeypatch.setattr("clio.split.get_duration_sec", lambda *a, **kw: 60.0)
        monkeypatch.setattr("clio.split.run_ffmpeg", lambda *a, **kw: None)
    ...
    def test_no_split_at_exact_max(self, monkeypatch, tmp_path: Path):
        monkeypatch.setattr("clio.split.get_duration_sec", lambda *a, **kw: 900.0)  # 15 min
        monkeypatch.setattr("clio.split.run_ffmpeg", lambda *a, **kw: None)
        result = split_video(tmp_path / "video.mp4", tmp_path / "splits", 15, "ffmpeg", "ffprobe")
        assert result == [tmp_path / "video.mp4"]

    def test_splits_into_two(self, monkeypatch, tmp_path: Path):
        monkeypatch.setattr("clio.split.get_duration_sec", lambda *a, **kw: 1200.0)  # 20 min
        ffmpeg_calls = []
        monkeypatch.setattr("clio.split.run_ffmpeg", lambda args, ff: ffmpeg_calls.append(args))
    ...
    def test_splits_with_exact_multiple(self, monkeypatch, tmp_path: Path):
        ...
        monkeypatch.setattr("clio.split.get_duration_sec", lambda *a, **kw: 1800.0)
        ffmpeg_calls = []
        monkeypatch.setattr("clio.split.run_ffmpeg", lambda args, ff: ffmpeg_calls.append(args))
    ...
    def test_segments_have_correct_format(self, monkeypatch, tmp_path: Path):
        monkeypatch.setattr("clio.split.get_duration_sec", lambda *a, **kw: 3600.0)
        monkeypatch.setattr("clio.split.run_ffmpeg", lambda *a, **kw: None)
    ...
    def test_last_segment_takes_remainder(self, monkeypatch, tmp_path: Path):
        ...
        monkeypatch.setattr("clio.split.get_duration_sec", lambda *a, **kw: 1350.0)
        ffmpeg_calls = []
        monkeypatch.setattr("clio.split.run_ffmpeg", lambda args, ff: ffmpeg_calls.append(args))
    ...
    def test_reencode_uses_video_codec(self, monkeypatch, tmp_path: Path):
        ...
        monkeypatch.setattr("clio.split.get_duration_sec", lambda *a, **kw: 1200.0)
        ffmpeg_calls = []
        monkeypatch.setattr("clio.split.run_ffmpeg", lambda args, ff: ffmpeg_calls.append(args))
