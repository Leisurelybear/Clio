...
**重要：每个 segment 的 index 字段必须精确匹配素材列表中某个素材的 index 值，不要自行编造序号。** 不要将 index 视为 segment 的序号，应将其视为引用素材的键。
...
REFINE_TEXT_PROMPT = """请审阅下面这段 vlog 素材的 AI 分析结果。**严格依据开头的「背景与规范」**修正其中的错误（例如地点误判、命名不一致、明显的常识错误、与上下文矛盾的描述），保留合理部分。
...
REFINE_SCRIPT_PROMPT = """请审阅下面这段 vlog 口播文案。**严格依据开头的「背景与规范」**修正其中的错误（如地名误用、景点混淆、时序/编号错误、风格不符），保留合理部分。
