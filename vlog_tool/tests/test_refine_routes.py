"""Tests for vlog_tool/ui/routes/refine.py — POST /api/refine handler."""
...
from vlog_tool.ui.routes.refine import _is_file_busy, _mark_busy, handle_post_refine
...
def _clear_busy_set():
    ...
    yield
    from vlog_tool.ui.routes.refine import _refining
...
class TestHandlePostRefineTextsSuccess:
    @patch("vlog_tool.ui.routes.refine.refine_text")
    @patch("vlog_tool.ui.routes.refine._save_atomic")
...
class TestHandlePostRefineScriptsSuccess:
    @patch("vlog_tool.ui.routes.refine.refine_script")
    @patch("vlog_tool.ui.routes.refine._load_analysis_for_script")
    @patch("vlog_tool.ui.routes.refine._save_atomic")
...
class TestHandlePostRefineError:
    ...
    def test_file_save_skipped_on_error(self, mock_refine):
        ...
        with patch("vlog_tool.ui.routes.refine._save_atomic") as mock_save:
            handle_post_refine(handler, {}, {"file": "test.json", "type": "texts"})
