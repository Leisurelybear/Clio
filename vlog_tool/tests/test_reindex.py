"""Tests for vlog_tool.tasks.reindex."""
...
from vlog_tool.config import AnalyzeConfig, AppConfig, PathsConfig
from vlog_tool.tasks.reindex import (
...
class TestAutoReindexIfNeeded:
    ...
    def test_all_indexed(self, config: AppConfig, compressed_dir: Path) -> None:
        ...
        with patch("vlog_tool.tasks.reindex.VideoIndex.read", return_value=MagicMock()) as mock_read:
...
class TestRunReindex:
    def test_missing_dir(self, config: AppConfig, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        with patch("vlog_tool.tasks.reindex.resolve_binary", return_value="ffprobe"):
    ...
    def test_writes_vindex_and_vmeta(self, config: AppConfig, compressed_dir: Path, input_dir: Path) -> None:
        ...
        with (
            patch("vlog_tool.tasks.reindex.VideoMeta.read", return_value=None),
            patch("vlog_tool.tasks.reindex.VideoMeta.build") as mock_build,
            patch("vlog_tool.tasks.reindex.get_duration_sec", return_value=60.0),
            patch("vlog_tool.tasks.reindex.resolve_binary", return_value="ffprobe"),
        ):
            mock_meta = MagicMock()
    ...
    def test_fallback_to_find_original(self, config: AppConfig, compressed_dir: Path, input_dir: Path) -> None:
        ...
        with (
            patch("vlog_tool.tasks.reindex.VideoMeta.read", return_value=None),
            patch("vlog_tool.tasks.reindex.VideoMeta.build") as mock_build,
            patch("vlog_tool.tasks.reindex._find_original_for_stem") as mock_find,
            patch("vlog_tool.tasks.reindex.get_duration_sec", return_value=60.0),
            patch("vlog_tool.tasks.reindex.resolve_binary", return_value="ffprobe"),
        ):
            mock_meta = MagicMock()
