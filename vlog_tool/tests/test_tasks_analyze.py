"""Tests for vlog_tool/tasks/analyze.py — run_analyze_all."""
...
def _common_mocks(monkeypatch):
    ...
    monkeypatch.setattr("vlog_tool.tasks.analyze.resolve_binary", lambda *a: "ffprobe")
    # Prevent AI calls
    monkeypatch.setattr("vlog_tool.tasks.analyze._build_stem", lambda idx, title, cfg: f"{idx:03d}_{title}")
    monkeypatch.setattr("vlog_tool.tasks.analyze._write_text_file", lambda *a: None)
...
class TestRunAnalyzeAll:
    ...
    def test_duration_gate_skips_long_video(self, monkeypatch, tmp_path: Path):
        ...
        _common_mocks(monkeypatch)
        monkeypatch.setattr("vlog_tool.tasks.analyze.get_duration_sec", lambda *a: 3600.0)
        analyze_called = False
