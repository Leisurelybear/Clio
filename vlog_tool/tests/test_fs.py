...
from vlog_tool.ui.routes.fs import _is_allowed_path, handle_get_fs_dirs
...
class TestHandleGetFsDirs:
    def test_empty_path_win32_returns_drives(self, monkeypatch):
        monkeypatch.setattr("sys.platform", "win32")
        monkeypatch.setattr("vlog_tool.ui.routes.fs._list_drives", lambda: ["C:\\", "D:\\"])
        handler = MagicMock()
    ...
    def test_valid_path_returns_sorted_dirs(self, tmp_path, monkeypatch):
        monkeypatch.setattr("vlog_tool.ui.routes.fs._is_allowed_path", lambda p: True)
    ...
    def test_path_traversal_returns_403(self, monkeypatch):
        monkeypatch.setattr("vlog_tool.ui.routes.fs._is_allowed_path", lambda p: False)
        handler = MagicMock()
    ...
    def test_permission_error_returns_403(self, monkeypatch):
        ...
        monkeypatch.setattr("vlog_tool.ui.routes.fs._is_allowed_path", mock_is_allowed)
