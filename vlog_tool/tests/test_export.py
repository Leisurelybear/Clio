"""Tests for vlog_tool.export package."""
...
from vlog_tool.export.jianying import (
...
class TestExportPlan:
    def test_jianying_delegates_to_exporter(self, tmp_path: Path) -> None:
        ...
        with patch("vlog_tool.export.FORMAT_REGISTRY", {"jianying": mock_fn}):
...
class TestResolveVideoByPrefix:
    ...
    def test_finds_video_by_prefix(self, tmp_path: Path) -> None:
        ...
        with (
            patch("vlog_tool.export.jianying.find_videos", return_value=[video]),
            patch("vlog_tool.export.jianying.get_duration_sec", return_value=60.0),
        ):
    ...
    def test_no_match_returns_none(self, tmp_path: Path) -> None:
        with (
            patch("vlog_tool.export.jianying.find_videos", return_value=[]),
        ):
            result = _resolve_video_by_prefix("001", tmp_path, "ffprobe")
...
class TestBuildMaterials:
    ...
    def test_builds_video_and_text(self, tmp_path: Path) -> None:
        ...
        with patch("vlog_tool.export.jianying._resolve_video") as mock_resolve:
    ...
    def test_skips_segment_when_video_not_found(self, tmp_path: Path) -> None:
        ...
        with patch("vlog_tool.export.jianying._resolve_video", return_value=None):
    ...
    def test_fallback_to_prefix_matching(self, tmp_path: Path) -> None:
        ...
        with patch("vlog_tool.export.jianying._resolve_video_by_prefix") as mock_prefix:
...
class TestBuildTracks:
    def test_builds_video_and_text_tracks(self) -> None:
        ...
        with patch("vlog_tool.export.jianying.parse_time_range") as mock_parse:
    ...
    def test_skips_invalid_timeline(self) -> None:
        ...
        with patch("vlog_tool.export.jianying.parse_time_range", side_effect=ValueError):
...
class TestExportPlanToJianying:
    ...
    def test_generates_complete_draft_json(self, tmp_path: Path) -> None:
        ...
        with (
            patch("vlog_tool.export.jianying.find_videos", return_value=[video_path]),
            patch("vlog_tool.export.jianying.get_duration_sec", return_value=60.0),
        ):
