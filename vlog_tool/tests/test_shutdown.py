...
import vlog_tool.shutdown as shutdown_mod
...
def test_before_stop_idempotent():
    with patch("vlog_tool.ai.factory._clear_provider_cache") as mock_clear:
...
def test_before_stop_terminates_alive_processes():
    ...
    with patch("vlog_tool.ai.factory._clear_provider_cache"):
        shutdown_mod.before_stop()
...
def test_install_hooks_idempotent():
    with (
        patch("vlog_tool.shutdown.atexit") as mock_atexit,
        patch("vlog_tool.shutdown.signal") as mock_signal,
    ):
...
def test_install_hooks_signal_sigterm():
    with (
        patch("vlog_tool.shutdown.atexit"),
        patch("vlog_tool.shutdown.signal") as mock_signal,
    ):
        shutdown_mod.install_hooks()
...
def test_before_stop_ignores_clear_cache_error():
    ...
    with patch("vlog_tool.ai.factory._clear_provider_cache", side_effect=Exception("fail")):
        shutdown_mod.before_stop()
