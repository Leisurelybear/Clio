...
class TestRunSubprocess:
    ...
    @patch("vlog_tool.utils.subprocess.run")
...
class TestPopenSubprocess:
    ...
    @patch("vlog_tool.utils.subprocess.Popen")
...
class TestDiscoverFfmpegBin:
    @patch("vlog_tool.utils.shutil.which", return_value=None)
    @patch("vlog_tool.utils.Path.is_dir", return_value=False)
    def test_returns_none_when_not_found(self, mock_is_dir, mock_which):
        result = discover_ffmpeg_bin("ffmpeg")
        assert result is None

    @patch("vlog_tool.utils.shutil.which", return_value="/usr/bin/ffmpeg")
    def test_returns_which_result(self, mock_which):
        result = discover_ffmpeg_bin("ffmpeg")
        assert result == "/usr/bin/ffmpeg"

    @patch("vlog_tool.utils.shutil.which", return_value=None)
    @patch("vlog_tool.utils.Path.is_dir", return_value=True)
    @patch("vlog_tool.utils.Path.glob")
    ...
    @patch("vlog_tool.utils.shutil.which", return_value=None)
    @patch("vlog_tool.utils.Path.is_dir", return_value=True)
    @patch("vlog_tool.utils.Path.glob", return_value=[])
...
class TestGetDurationSec:
    def test_returns_float(self):
        with patch("vlog_tool.utils.run_subprocess") as mock_run:
...
class TestWriteJsonAtomic:
    ...
    def test_cleanup_on_crash(self, tmp_path):
        f = tmp_path / "test.json"
        with patch("vlog_tool.utils.os.replace", side_effect=OSError("rename failed")):
...
class TestRunFfmpeg:
    @patch("vlog_tool.utils.popen_subprocess")
